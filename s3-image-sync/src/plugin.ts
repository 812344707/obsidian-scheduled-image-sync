...
import { ImageCleanup } from "./image-cleanup";
import { ImageCleanupModal } from "./image-cleanup-modal";
...
export default class S3ImageSyncPlugin extends Plugin {
  ...
  private busyNotes = new Set<string>();
  private localDeletionCount = 0;
  cleanup!: ImageCleanup;

  get syncBusy(): boolean { return this.autoScanRunning || this.busyNotes.size > 0 || this.localDeletionCount > 0; }
  ...
  async runAutoScan(): Promise<void> {
    if (this.stopped || this.cleanup.running || this.isMobile || this.autoScanRunning || !this.settings.enabled || !this.settings.autoScanEnabled) return;
  ...
  private async runAutoScanOnce(): Promise<void> {
    try {
      this.ensureS3Settings();
    } catch {
      return;
    ...
    for (const file of files) {
      if (this.stopped || !this.settings.enabled || !this.settings.autoScanEnabled) break;
      try {
  ...
  async replaceCandidates(
    ...
    if (this.stopped || this.cleanup.running || this.busyNotes.has(noteFile.path)) return { replaced: 0 };
  ...
  private async replaceCandidatesOnce(
    noteFile: TFile,
    candidates: Candidate[],
    progress: ((state: ProgressState) => void) | null,
    options: { deleteMode?: DeletePolicy }
    ...
    let replaced = 0;
    const replacements: Array<{ ref: LocalRef; text: string }> = [];
    ...
    try {
      ...
      // A content-addressed object may already be used by another note.
      // Keep uploaded objects on a failed rewrite; never delete shared remote images.
      throw error;
    }
  ...
  async deleteLocalFileRecords(
    ...
    if (this.cleanup.running) throw new Error(this.t("cleanupBusy"));
    ...
    await this.saveSettings();
    } finally { this.localDeletionCount--; }
  }
