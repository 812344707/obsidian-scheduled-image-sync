...
interface LoadedSettings {
  uploadRecords?: PluginSettings["uploadRecords"];
  enabled?: boolean;
  ...
  deletePolicy?: PluginSettings["deletePolicy"];
...
export function mergeSettings(defaults: PluginSettings, loaded: unknown): PluginSettings {
  ...
  return {
    ...
    uploadRecords: Array.isArray(data.uploadRecords) ? data.uploadRecords.filter(r =>
      r && [r.sourcePath, r.sourceHash, r.key, r.publicUrl, r.endpoint, r.bucketName, r.region, r.uploadedAt]
        .every(value => typeof value === "string" && value.length > 0)
    ) : [],
  };
}

export function normalizeScanInterval(value: unknown): number {
  const minutes = Number(value);
  return Number.isFinite(minutes) && minutes > 0
    ? Math.min(10080, Math.max(1, minutes))
    : 30;
}
